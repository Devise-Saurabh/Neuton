...
{
  uint16_t u16IndexDtc;
  uint8_t u8FaultExists = deMCD_FALSE;
  
  /*Loop on all the DTCs*/
  for(u16IndexDtc = (uint16_t)0; u16IndexDtc < APP_FLT_DTC_NUM; u16IndexDtc++)
  {
    /*If there is an error read fucntion*/
    if(FLT_astrDtc[u16IndexDtc].p2fRteReadError != deMCD_NULL)
    {
      /* Check if there is or there is not an error*/
      u8FaultExists = (uint8_t)FLT_astrDtc[u16IndexDtc].p2fRteReadError();
      
      /*If there is an error*/
      if((u8FaultExists == deMCD_TRUE) && (FLT_astrDtc[u16IndexDtc].eDtcSeverityDecided != FLT_astrDtc[u16IndexDtc].eDtcSeverity))
      {       
        ...
        else
        {
          ...
          if((FLT_astrDtc[u16IndexDtc].u32DeltaTimeCounter * APP_FLT_TASK_MS) >= FLT_astrDtc[u16IndexDtc].u16ResetDeltaTimeMs)
          {
            ...
            FLT_astrDtc[u16IndexDtc].p2fRteWriteErrorClear(deMCD_TRUE);
          }
          else
          {
            FLT_astrDtc[u16IndexDtc].p2fRteWriteErrorClear(deMCD_FALSE);
          }
        }
      }
      else
      {
        if(u8FaultExists == deMCD_FALSE)
        {
          FLT_astrDtc[u16IndexDtc].p2fRteWriteErrorClear(deMCD_FALSE);
...
void app_FaultMgr_ClearDTC(void)
{
  ...
  {
    FLT_astrDtc[u8Counter].u32DeltaTimeCounter = deMCD_DEFAULT;
    FLT_astrDtc[u8Counter].u8ErrorFlag = deMCD_FALSE;
    FLT_astrDtc[u8Counter].u8ErrorCount = deMCD_DEFAULT;
